...
export const useBoardState = (boardNum: number) => {
  ...
  useEffect(() => {
    if (!newWord && !loadingWordOfTheDay) {
      const storageBoardStateStr = localStorage.getItem(`boardState_${boardNum}`);
      ...
      const guessIndex = localStorage.getItem(`currentGuessIndex_${boardNum}`);

      if (guessIndex) {
        setCurrentGuessIndex(Number(guessIndex));
      }

      const guessRowIndex = localStorage.getItem(`currentGuessRow_${boardNum}`);

      if (guessRowIndex) {
        setCurrentGuessRow(Number(guessRowIndex));
      }

      const storageGameStatus = localStorage.getItem(`gameStatus_${boardNum}`);

      if (storageGameStatus) {
        setGameStatus(storageGameStatus as GameStatus);
      }

      const gw = localStorage.getItem(`gameWon_${boardNum}`);
      if (typeof gw === "string") {
        setGameWon(gw === "true");
      }
    }
  }, [boardNum, loadingWordOfTheDay]);

  // Save game state after each guess
  useEffect(() => {
    if (!loadingWordOfTheDay) {
      localStorage.setItem(`boardState_${boardNum}`, JSON.stringify(boardState));
    }
  }, [boardNum, guess0, guess1, guess2, guess3, guess4, guess5, guess6, loadingWordOfTheDay]);

  // Save game state after each guess
  useEffect(() => {
    if (!loadingWordOfTheDay) {
      localStorage.setItem(`currentGuessIndex_${boardNum}`, currentGuessIndex.toString());
    }
  }, [boardNum, currentGuessIndex, loadingWordOfTheDay]);

  // Save game state after each guess
  useEffect(() => {
    if (!loadingWordOfTheDay) {
      localStorage.setItem(`currentGuessRow_${boardNum}`, currentGuessRow.toString());
    }
  }, [boardNum, currentGuessRow, loadingWordOfTheDay]);

  // Save game state after each guess
  useEffect(() => {
    if (!loadingWordOfTheDay) {
      localStorage.setItem(`gameStatus_${boardNum}`, gameStatus);
    }
  }, [boardNum, gameStatus, loadingWordOfTheDay]);

  // Save game state after each guess
  useEffect(() => {
    if (!loadingWordOfTheDay) {
      localStorage.setItem(`gameWon_${boardNum}`, gameWon.toString());
    }
  }, [boardNum, gameWon, loadingWordOfTheDay]);
